...
export const Route = createFileRoute("/vehicles/browse")({
  ...
  head: () => ({
    meta: [
      { title: "Browse Vehicles — Spott.ca" },
...
function BrowsePage() {
  ...
  const filters = useMemo(() => ({
    ...
    city: city || undefined,
  }), [q, make, model, yearMin, priceMax, body, sellerType, city]);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="mt-5 grid gap-3 rounded-xl border border-border bg-card p-4 sm:grid-cols-2 md:grid-cols-4">
        <input className="rounded-md border border-border bg-background p-2 text-sm" placeholder="Search…" value={q} onChange={(e) => setQ(e.target.value)} />
            ...
            return (
              <Link key={v.id} to="/vehicles/$id" params={{ id: v.id }} className="group flex flex-col overflow-hidden rounded-xl border border-border bg-card transition hover:border-primary/40">
                <div className="relative aspect-[4/3] overflow-hidden bg-muted">
                  {img ? (
                    <img src={img} alt={v.title} loading="lazy" className="h-full w-full object-cover transition group-hover:scale-105" />
                  ) : (
                    <div className="flex h-full items-center justify-center text-muted-foreground"><Car className="h-10 w-10" /></div>
                  )}
                  <span className="absolute left-2 top-2 rounded-full bg-background/90 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide backdrop-blur">
                    {v.seller_type}
                  </span>
                </div>
                <div className="p-3">
                  <div className="text-base font-semibold">{fmtPrice(v.price_cents, v.currency)}</div>
                  <div className="line-clamp-1 text-sm text-foreground">{v.title}</div>
